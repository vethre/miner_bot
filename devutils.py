...
SOURCE_CHANNEL_ID = -1001976943087  
TARGET_GROUP_ID = -1001603288596
...
async def handle_channel_post(message: types.Message):
    ...
    if message.chat.id == SOURCE_CHANNEL_ID:
        try:
            ...
            logging.info(f"Сообщение {message.message_id} успешно переслано в группу {TARGET_GROUP_ID}")
        except Exception as e:
            logging.error(f"Ошибка при пересылке сообщения {message.message_id}: {e}")
            # Опционально: можно отправить сообщение об ошибке админу или в специальный лог-канал
